...
def main():
    ...
    # multi use the below:
    configs = json.load(open('640-80-5-layers-multi-revise1.json', 'r'))
    
    # single with 1024 batchsize use below:
    # configs = json.load(open('640-80-2-layers.json', 'r'))
    ...
    #print(math.ceil((data.len_train - configs['data']['sequence_length']) // configs['training']['batch_size']))
    
# out-of memory generative training
    steps_per_epoch = math.ceil((data.len_train - configs['data']['sequence_length']) // configs['training']['batch_size'])
    model.train_generator(
        data_gen=data.generate_train_batch(seq_len=configs['data']['sequence_length'], batch_size=configs['training']['batch_size'], normalise=configs['data']['normalise']),
        epochs=configs['training']['epochs'],
        batch_size=configs['training']['batch_size'],
