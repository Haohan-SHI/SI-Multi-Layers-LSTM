...
class Model():
	...
	def train(self, x, y, epochs, batch_size, save_dir):
		...
		callbacks = [
			...
			TensorBoard(log_dir='./logs', write_graph=True, write_images=True, write_steps_per_second=False)
		]
		self.model.fit(
			x,
			y,
	...
	def train_selfmade(self, x, y, epochs, batch_size, save_dir, steps_per_epoch, initial_epoch):
		timer = Timer()
		timer.start()
		print('[Model] Training Started')
	...
	def predict_point_by_point(self, data,debug=False):
		if debug == False:
			...
			print('[Model] Predicting Point-by-Point...')
			print (np.array(data).shape)  # (6153, 49, 1)
			predicted = self.model.predict(data)
			print (np.array(predicted).shape) # (6153, 1)
			predicted = np.reshape(predicted, (predicted.size,))
			print (np.array(predicted).shape) # (6153, )
		return predicted
